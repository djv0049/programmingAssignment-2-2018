...
def setup():
    ...
    get_results(a_competition)
    return a_competition

def get_results(a_competition):
    results_data = json.loads(open('BETTERresultsJSON.txt').read())
    all_results = results_data['results']
    for result in all_results:
        a_competition.set_result(result['homeTeamRank'],result['homeTeamScore'], result['homeTeamTries'], result['awayTeamRank'], result['awayTeamScore'], result['awayTeamTries'])
