/* globals Competition */
class Controller { // eslint-disable-line no-unused-lets

  static getJSONResults(){
    return{
      'results': [
        {
            "week":1,
  ...
  static setup () {
    ...
    this.getResults(the2018Competition)

    return the2018Competition
  }
  static getResults(the2018Competition){
    let resultData = Controller.getJSONResults()
    let allResults = resultData.results
    for (let aResult of allResults){
      the2018Competition.setResult(aResult.homeTeamRank, aResult.homeTeamScore, aResult.homeTeamTries, aResult.awayTeamRank, aResult.awayTeamScore, aResult.awayTeamTries)
    }
  }
}
